    ...
    <div className="min-h-screen">
      ...
      <section className="relative bg-ink-900 px-6 py-16 pb-44">
        <div className="mx-auto max-w-2xl">
          ...
          <Reveal>
            <div className="relative overflow-hidden rounded-3xl border border-champagne-400/30 bg-gradient-to-br from-ink-800/90 via-ink-800/60 to-champagne-950/20 p-7 sm:p-9 backdrop-blur-md shadow-2xl transition-all duration-300 hover:border-champagne-400/60">
          ...
          {/* Know Your Host */}
          <Reveal delay={200}>
            <div className="mt-12 overflow-hidden rounded-3xl border border-ink-700 bg-ink-800/40 p-6 sm:p-8 backdrop-blur-md transition-colors hover:border-champagne-500/30">
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-5 sm:gap-6">
                <div className="relative shrink-0">
                  <img
                    src={property.host.image}
                    alt={property.host.name}
                    className="h-20 w-20 sm:h-24 sm:w-24 rounded-full border-2 border-champagne-400/40 object-cover shadow-xl"
                  />
                  <span className="absolute bottom-1 right-1 h-3.5 w-3.5 rounded-full border-2 border-ink-900 bg-olive-400" title="On-site" />
                </div>
                <div className="flex-1">
                  ...
                  <p className="text-xs text-stone-400">{property.host.title}</p>
                </div>
                <a
                  href={`tel:${property.host.phone.replace(/[^0-9+]/g, '')}`}
                  className="no-tap-highlight inline-flex items-center gap-2 rounded-full border border-champagne-400/30 bg-champagne-500/10 px-4 py-2 text-xs font-medium uppercase tracking-wider text-champagne-300 transition-colors hover:bg-champagne-500 hover:text-ink-900"
                >
          ...
          <Reveal delay={300}>
            <div className="mt-16 space-y-px">
              {[
                ...
                { label: 'House guide & essentials', view: 'guide', desc: 'WiFi credentials, taxis, WhatsApp delivery & rules' },
