...
import { ChevronRight, ArrowRight, ArrowLeft, MapPin, Clock, Lightbulb, X } from 'lucide-react';
import { amenities, propertyStory, residents, tourStops } from '@/data/content';
  ...
  if (tourIndex !== null) {
    ...
    return (
      <div className="fixed inset-0 z-50 bg-ink-900 flex flex-col justify-between overflow-hidden animate-fade-in">
        ...
        {/* Top left Exit button matching all other pages */}
        <div className="absolute top-6 left-6 z-30">
          <BackButton onClick={() => setTourIndex(null)} label="Exit" />
        </div>

        {/* Center Content */}
        <div className="relative z-20 flex flex-col items-center justify-center px-6 text-center max-w-xl mx-auto my-auto">
          <p className="font-serif text-6xl sm:text-7xl font-light text-champagne-300/90">
            {stop.number}
          </p>
          <h2 className="mt-4 font-serif text-4xl font-light text-ivory-50 hero-text-shadow sm:text-5xl">
            {stop.title}
          </h2>
          <p className="mt-5 font-serif text-lg font-light italic text-ivory-200/90 hero-text-shadow leading-relaxed">
            {stop.description}
          </p>
        </div>

        {/* Bottom Navigation Bar */}
        <div className="relative z-20 flex items-center justify-between p-6 sm:p-8 border-t border-ivory-200/10 bg-ink-900/60 backdrop-blur-md">
          {/* Previous Stop */}
          <button
    ...
    <div className="min-h-screen bg-ink-900">
      ...
      <section className="px-6 py-10 bg-ink-800/40 border-y border-ink-700">
        <div className="mx-auto max-w-3xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            <p className="text-xs uppercase tracking-widest-2 text-champagne-400 font-medium">Digital Tour</p>
            <p className="font-serif text-xl font-light italic text-ivory-100 mt-1">Take a self-guided tour of the estate grounds.</p>
          </div>
          <button
            onClick={() => setTourIndex(0)}
            className="no-tap-highlight flex items-center gap-2 rounded bg-champagne-500/90 px-5 py-3 text-xs uppercase tracking-widest-2 font-medium text-ink-900 transition-colors hover:bg-champagne-400 shadow-md"
          >
      ...
      <section className="px-6 py-16">
        <div className="mx-auto max-w-3xl">
          <Reveal>
            <p className="text-xs uppercase tracking-widest-3 text-stone-500">Amenities</p>
          </Reveal>
      ...
      {/* Property Story */}
      <section className="px-6 py-16 bg-ink-800/30 border-t border-ink-700">
        <div className="mx-auto max-w-3xl">
          <Reveal>
